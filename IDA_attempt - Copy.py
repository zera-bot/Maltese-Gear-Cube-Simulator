...
def heuristic(cube: Cube):
    # evaluate the heuristic property of a given state - closer to 0 is closer to the goal
    #   for each of the moves, figure out which one affects how many gears and corners
    #   we want the algorithm to move exactly 1 gear and 0 corners within the range of a move
    min_heuristic = INF
    for move in ["R","L","U","D","F","B"]:
        cornersAffected = turnData[move]["corners"]
        gearsAffected = [ x for xs in turnData[move]["gears"] for x in xs ]
        gearsMoved = sum(1 for gear in gearsAffected if gear != cube.gears[gear].initialPosition)
        cornersMoved = sum(1 for corner in cornersAffected if corner != cube.corners[corner].initialPosition)

        min_heuristic = min(min_heuristic, abs(gearsMoved-1)+cornersMoved)
